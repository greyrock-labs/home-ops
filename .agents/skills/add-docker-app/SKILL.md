---
name: add-docker-app
description: Use when deploying or changing an application on the docker hosts (codswallop, skedaddle) — docker-compose stacks under docker/, doco-cd deployments, "deploy X to the NAS", secrets or reverse-proxy wiring for a compose app
---
...
- **Publishing HTTP ports on skedaddle** — everything HTTP goes through caddy-l4 over `edge`; published ports bypass TLS and auth.
